...
plugin = Plugin(name="/реши [выражение] - посчитать результат с помощью калькулятора\n/реши <помощь> - помощь по калькулятору")
...
@plugin.on_startswith_text("реши")
async def calc(message, attachments, env):
    if not env.body:
        return await env.reply("Нечего считать! Пожалуйста, укажите выражение!")

    if env.body.lower() == "помощь":
        result = ""
        ...
        for k, v in FUNCTIONS.items():
            ...
            # env.reply делит сообщения на части длиной 550, но нам
            # не надо, чтобы описание одного плагина разошлось
            # на несколько строк
            if len(result) + len(temp) >= 550:
                await env.reply(result)
                result = ""
        ...
        if current_exp in FUNCTIONS:
            ...
            if current_exp == ")":
                if not do_bracket(data):
                    return await env.reply("Ошибка в разборе выражения!")
    ...
    if len(data.Q) != 1:
        ...
        return await env.reply(f"Ответ: {data.Q.pop()}")
